...
export function reactionNext(reaction: string): string {
  return ({ new: 'Zacznij od próby opisanej wyżej. Jej wynik pokaże, czy ten trop pasuje do Twojego dnia.', known: 'Masz teraz moment działania oraz warunek oceny. Wróć po próbie do „Co wyszło”, żeby wybrać kolejny ruch.', obvious: 'Nie powtarzaj tej samej rady w ciemno. Popraw odpowiedź o poprzedniej próbie i jej wyniku. Jeśli już je podałeś, pobierz zapis: automat nie ma dość szczegółów, by zaproponować kolejną korektę.', off: 'Ten trop nie pasuje. Wróć do sceny i tego, co było wcześniej. Jeśli żadna odpowiedź nie opisuje sytuacji, wybierz „Nie wiem albo było inaczej”, zamiast dopasowywać siebie do wyniku.' } as Record<string, string>)[reaction] || '';
}
