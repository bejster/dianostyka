...
export default function ResultExperience({
  ...
  // BEAT 8: intent-aware next step -> nabor. Kazdy poziom gotowosci = inne copy i inna sila CTA.
  // Tier A i in_sam NIE dostaja hard pitchu (soft). in_prowadz = najmocniejszy CTA.
  const INTENT_NEXT: Record<string, { kicker: string; h2: string; body: string; cta: string; variant: 'strong' | 'medium' | 'soft' }> = {
    in_prowadz: {
      kicker: 'Napisałeś, że wolisz prowadzenie',
      h2: 'Teraz już wiesz, jak patrzę na taki wynik.',
      body: 'Jeśli chcesz, żebym robił to z Twoim tygodniem na bieżąco, tutaj zobaczysz cały proces prowadzenia, zakres współpracy i jak wygląda wejście.',
      cta: 'Zobacz prowadzenie 1:1 →', variant: 'strong',
    },
    in_zobacz: {
      kicker: 'Chciałeś zobaczyć, jak wygląda praca z kimś',
      h2: 'Ta diagnostyka była małą próbką mojego sposobu pracy.',
      body: 'Na stronie prowadzenia zobaczysz, co dzieje się po takim wyniku, jak wygląda cotygodniowa korekta i co dokładnie dostajesz przez 6 miesięcy.',
      cta: 'Zobacz, jak wygląda prowadzenie →', variant: 'medium',
    },
    in_sam: {
      kicker: 'Chcesz najpierw ograć to sam',
      h2: 'Zrób test i zobacz, co pokaże tydzień.',
      body: 'Jeśli po tygodniu nadal będziesz wracał do tego samego miejsca albo nie będzie wiadomo, co ruszyć dalej, tutaj możesz zobaczyć, jak wygląda praca ze mną.',
      cta: 'Zobacz na później →', variant: 'soft',
    },
    in_niewiem: {
      kicker: 'Nie musisz teraz nic decydować',
      h2: 'Masz już pierwszy test. To wystarczy na dziś.',
      body: 'Jeśli chcesz tylko zobaczyć, co dzieje się dalej w prowadzeniu, tutaj masz cały proces bez podejmowania decyzji teraz.',
      cta: 'Zobacz, jak to wygląda →', variant: 'medium',
    },
  };
  const nextStep = tier === 'A'
    ? { kicker: 'Nie ma pośpiechu', h2: 'Na tym wyniku nie widzę powodu, żeby od razu dokładać Ci prowadzenie.', body: `Najpierw sprawdziłbym ten test przez ${durLabel}. Jakbyś potem chciał zobaczyć, jak wygląda praca ze mną, cały proces masz tutaj.`, cta: 'Zobacz na później →', variant: 'soft' as const }
    : (INTENT_NEXT[intent] || INTENT_NEXT.in_niewiem);
    ...
    <div className="rx" style={{ background: C.ink, color: C.paper, fontFamily: C.sans, minHeight: '100vh', overflowX: 'hidden', position: 'relative' }}>
      ...
      <div className="rx-wrap">
        ...
        <section className="rx-beat" data-beat="2">
          ...
          <p className="rx-sub">To pierwszy moment, od którego reszta tygodnia najczęściej zaczyna lecieć gorzej. Za chwilę pokażę Ci, co się z nim najprawdopodobniej łączy.</p>
        </section>
        ...
        <section className="rx-beat" data-beat="6">
          <div className="rx-kick">Jedno pytanie na koniec</div>
          ...
          {calib && <p className="rx-fine" style={{ textAlign: 'left', marginTop: 16 }}>Dzięki. To jest dokładnie informacja, której potrzebuję, żeby następna decyzja była lepsza.</p>}
        </section>
        ...
        <section className="rx-beat" data-beat="7">
          ...
          <h2 className="rx-h2" style={{ fontSize: 'clamp(24px,5vw,38px)', marginBottom: 16 }}>To, co właśnie zobaczyłeś na swoim wyniku, w prowadzeniu robię z Twoim tygodniem co 7 dni.</h2>
          {el && <p className="rx-sub" style={{ marginBottom: 18, color: C.paper }}>{el}</p>}
          <p className="rx-sub" style={{ marginBottom: 22 }}>Dzisiaj to jeden wycinek tygodnia. Za tydzień może zmienić się sen, praca, trening, weekend albo to, co właśnie testujesz. Wtedy ten sam wynik już nie wystarcza. Liczy się kolejna decyzja na podstawie tego, co faktycznie się wydarzyło.</p>
          ...
          <div className="rx-wk-head">Twój tydzień nie stoi w miejscu. Decyzje też nie powinny.</div>
        ...
        <section className="rx-beat" data-beat="8">
          ...
          <a className={'rx-next rx-next-' + nextStep.variant} href={ctaHref} target={ctaHref.startsWith('http') ? '_blank' : undefined} rel="noopener noreferrer" onClick={() => trackDiag('nabor_click', { archetype: archKey, tier, intent, cta_variant: nextStep.variant })}>{nextStep.cta}</a>
          ...
          {instagram && <p className="rx-fine">Ta diagnoza jest tylko Twoja, pod @{instagram}. Trafia wyłącznie do mnie.</p>}
        </section>
