...
// Tylko produkcja zasila lejek. Localhost, preview i testy headless pisaly do tej samej kohorty
// co prawdziwy ruch (2026-09-26), wiec kazda zmiana wygladu mierzyla tez moje przeklikania.
// NEXT_PUBLIC_POSTHOG_FORCE=1 wlacza swiadomie poza produkcja.
const PH_ON = process.env.VERCEL_ENV === 'production' || process.env.NEXT_PUBLIC_POSTHOG_FORCE === '1';
...
const DXLDR_CSS = `
.dxldr{display:none}
html.dxldr-on .dxldr{display:flex;position:fixed;inset:0;z-index:9999;background:radial-gradient(120% 90% at 50% 42%,#12100c 0%,#08080a 62%);align-items:center;justify-content:center;pointer-events:none;animation:dxldrOut .6s cubic-bezier(.22,1,.36,1) 0.35s forwards}
...
html.dxldr-on .dxldr-in{position:relative;text-align:center;animation:dxldrLift .6s cubic-bezier(.22,1,.36,1) 0.35s forwards}
...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  return (
    <html lang="pl" suppressHydrationWarning>
      <head>
        {/* P0-1 (rc-004): opaque lead_ref z FRAGMENTU URL (#rid=/#lead_ref=). Fragment NIE leci w HTTP requeście do Vercela,
            wiec token nie trafia do logow serwera. Ten bootstrap MUSI byc PRZED PostHogiem i Meta Pixelem: czyta token,
            chowa do sessionStorage (prywatny klucz), NATYCHMIAST usuwa z URL (replaceState), dopiero potem startuja trackery.
            Efekt: token nigdy nie widoczny dla product/marketing analytics, tylko dla prywatnej sciezki leada (page.tsx -> lead-notify). */}
        <script dangerouslySetInnerHTML={{__html:`
try{var h=location.hash||"";var m=h.match(/[#&](?:rid|lead_ref)=([A-Za-z0-9_-]{6,64})(?:&|$)/);if(m){try{sessionStorage.setItem("diag_lead_ref",m[1])}catch(e){}var clean=h.replace(/([#&])(?:rid|lead_ref)=[A-Za-z0-9_-]+/,"$1").replace(/[#&]+$/,"");try{history.replaceState(null,"",location.pathname+location.search+(clean&&clean!=="#"?clean:""))}catch(e){}}}catch(e){}
        `}} />
        {/* PostHog — lejek. capture_pageview OFF: brak auto-pageview (privacy+ordering), uzywamy jawnych eventow diagnostyki. */}
        {PH_ON && PH_KEY && (
          <script dangerouslySetInnerHTML={{__html:`
...
posthog.init(${JSON.stringify(PH_KEY)},{api_host:${JSON.stringify(PH_HOST)},person_profiles:'identified_only',capture_pageview:false,autocapture:false,capture_heatmaps:false,disable_session_recording:true,mask_all_text:true,mask_all_element_attributes:true});
          `}} />
        )}
      ...
      <body>
        ...
        <div className="dxldr" aria-hidden="true">
          <div className="dxldr-in">
            ...
            <span className="dxldr-cap">PRZEGLĄD TWOJEGO TYGODNIA</span>
          </div>
        </div>
        {children}
      </body>
